...
export function diffRevisions(base, other, includes) {
    ...
    const filteredBaseData = baseDataPromise.then((baseData)=>{
		return diffFilter(baseData.toJSON());;
	})

	if (!other) {
		 return diff(
				{},
				filteredBaseData ? sortEntityData(filteredBaseData) : {}
			)
	}

	const otherDataPromise = other.related('data').fetch({require: false, withRelated: includes});
    const filteredOtherData = otherDataPromise.then((otherData)=>{
		return diffFilter(otherData.toJSON());;
	})

	return Promise.all([filteredBaseData, filteredOtherData])
